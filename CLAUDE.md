...
- **The draw-call load is the individually-`Node`d meshes, not the instanced scenery.** Every `Node` carrying a `Mesh` is its own draw call, and composite props multiply: a tree is 4 meshes, a barrier 8, a container 8. Road tiles, lane dashes, coins, particles and houses have all been converted (178 draw calls → 16). **Trees (72) and pooled obstacles (~72) are what is left**, against ~30 for every instanced set in the scene combined.

  The conversion recipe, now used three times (coins, particles, houses) — follow it rather than reinventing:
  1. Drop the per-object `Node`; the object's index in its pool becomes its instance index.
  2. A batch binds **one material**, so anything that varies by colour needs one mesh per colour. Particles split their burst colours into pools; houses have one roof mesh per roof colour. An empty `InstancedMesh` costs nothing, so over-provisioning colour buckets is free.
  3. Composite props compose `propWorld * partLocal` per part per frame (`_compose` in the painter, with the part-local matrices built once as `static final`). Keep the prop's slot inside a *colour-specific* mesh separately — it is not the same as its index in the prop list (see `_houseRoofSlot`).
  4. Inactive pooled instances collapse to a **zero scale**, never `y = -1000`. Parking would stretch the set's aggregate bounds and stop the whole batch frustum-culling.

  Trees are the same shape of problem as houses, just wider: 6 distinct foliage geometries (3 cone tiers for pines, centre sphere + two blob radii for round crowns) × 4 foliage colours, plus a shared trunk mesh — roughly 23 draw calls instead of 72. Obstacles add pooled active/inactive state on top of that.
